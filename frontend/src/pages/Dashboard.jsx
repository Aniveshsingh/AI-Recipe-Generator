  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setStats({
          totalRecipes: statsData.recipes.total_recipes,
          pantryItems: statsData.pantry.total_items,
          mealsThisWeek: statsData.mealPlans.this_week_count,
        });

        // 🔥 Fetch recipes
        const recipeRes = await apiFetch("/recipes");
        const recipeData = await recipeRes.json();

        setRecentRecipes(
          recipeData.slice(0, 5).map((r) => ({
            ...r,
            id: r._id,
          })),
        );
        ...
        // 🔥 Fetch meal plans
        const mealRes = await apiFetch("/mealplan");
        const mealData = await mealRes.json();

        setUpcomingMeals(
          mealData.slice(0, 5).map((m) => ({
            ...m,
            id: m._id,
          })),
        );
